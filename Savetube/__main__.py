...
class CliApp:
    ...
    def __init__(self, url):
        ...
        def getJson():
            ...
            if y2j.getFormatsExt()[(ext -1)] == "mp3":
                ...
                os.system("youtube-dl -o \"{}/%(title)s.%(ext)s\" -x --audio-format mp3 {}".format(get_download_path(), url))
                ...
                os.system("youtube-dl --rm-cache-dir")
                os.system("youtube-dl  -o \"{}/%(title)s.%(ext)s\" {} -f \"bestvideo[height<={}][ext={}]+bestaudio/best\" {}".format(
    ...
    def intro(self):
        ...
        f = Figlet(font='slant')
        r = pyfiglet.figlet_format(" SaveTube", font = "banner")
        cprint(f.renderText(" SaveTube"), 'white', 'on_red', attrs=['bold'])

        
        print(colored("ffmpeg_version: {}".format(self.video_converter['ffmpeg']), 'green' if self.video_converter['ffmpeg'] else 'red'), end="\t\t")
        print(colored("avconv_version: {}".format(self.video_converter['avconv']), 'green' if self.video_converter['avconv'] else 'red'), end="\t\t")
